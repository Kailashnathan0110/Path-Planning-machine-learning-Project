...
from differentableAStar import AstarOutput
...
def solve_single(pred_cost: np.array,start_map: np.array,goal_map: np.array,map_design: np.array,g_ratio: float = 0.5,) -> list:
...
def pq_astar(pred_costs: np.array,start_maps: np.array,goal_maps: np.array,map_designs: np.array,
             store_intermediate_results: bool = False,g_ratio: float = 0.5,) -> AstarOutput:
    assert (
